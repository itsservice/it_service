...
function mapTicket(t) {
  return {
    ...
    reporter:     t.reporter      || t.reporter || '',
    ...
    engineerName: t.engineer_name || t.engineer_name || '',
    workDetail:   t.work_detail   || t.work_detail || '',
    partsUsed:    t.parts_used    || t.parts_used || '',
    workHours:    t.work_hours    || t.work_hours || '',
    lineUserId:   t.line_user_id  || t.line_user_id || '',
    createdAt:    t.created_at    || t.created_at || '',
    sla:          t.sla           || t.sla || '',
    // fields ที่ FastAPI ไม่มี — เก็บ local
    adminNote:    t.admin_note    || t.admin_note || '',
    closedAt:     t.closed_at     || t.closed_at || '',
    closedBy:     t.closed_by     || t.closed_by || '',
    completedAt:  t.completed_at  || t.completed_at || '',
  };
}
